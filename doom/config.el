...
(let ((serif-font doom-serif-font))
    (custom-set-faces!
     `(org-document-title :inherit (variable-pitch) :font ,serif-font :height 1.5 :weight bold)
     `(org-level-1       :inherit (variable-pitch) :font ,serif-font :height 1.4 :weight bold)
     `(org-level-2       :inherit (variable-pitch) :font ,serif-font :height 1.3 :weight bold)
     `(org-level-3       :inherit (variable-pitch) :font ,serif-font :height 1.2 :weight bold)
     `(org-level-4       :inherit (variable-pitch) :font ,serif-font :height 1.1 :weight bold)
     `(org-level-5       :inherit (variable-pitch) :font ,serif-font :weight bold)
     `(org-level-6       :inherit (variable-pitch) :font ,serif-font :weight bold))))
